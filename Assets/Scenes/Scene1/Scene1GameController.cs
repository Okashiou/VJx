...
public class Scene1GameController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            _material.SetFloat("_Size1", 1 + (Lasp.MasterInput.CalculateRMS(Lasp.FilterType.Bypass) * 100) % 1);
            _material.SetFloat("_Size2", 1 + (Lasp.MasterInput.CalculateRMS(Lasp.FilterType.HighPass) * 100) % 1);
